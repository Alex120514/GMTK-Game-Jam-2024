...
public class ResizeGrounds21 : MonoBehaviour
{
    ...
    IEnumerator ResizeAndRepositionGrounds()
    {
        ...
        for (int i = 0; i < grounds.Length; i++)
        {
            ...
            targetSizes[i] = new Vector3(size.x * 1.414f, size.y / 1.414f, size.z);
            targetPositions[i] = new Vector3(normalPositions[i].x * 1.414f, normalPositions[i].y / 1.414f, normalPositions[i].z);
        }
